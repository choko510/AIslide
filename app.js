...
document.addEventListener('DOMContentLoaded', function() {
        ...
        <div class="modal__overlay" tabindex="-1" data-micromodal-close>
            <div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="qr-modal-title">
                <header class="modal__header">
                    <h2 class="modal__title" id="qr-modal-title">QRコード生成</h2>
                    <button class="modal__close" aria-label="Close modal" data-micromodal-close></button>
                </header>
                <main class="modal__content" id="qr-modal-content">
                    <form id="qr-create-form">
                        <div style="margin-bottom:10px;">
                            <label>QRコード内容（URLやテキスト）</label>
                            <input type="text" id="qr-text" value="" required style="width:100%;">
                        </div>
                        <div style="margin-bottom:10px;">
                            <label>サイズ(px)</label>
                            <input type="number" id="qr-size" value="256" min="64" max="1024" step="8" style="width:100px;">
                        </div>
                        <div id="qr-preview" style="margin-bottom:10px;text-align:center;"></div>
                        <button type="submit" class="modal__btn modal__btn-primary" style="width:100%;">スライドに追加</button>
                    </form>
                </main>
            </div>
        </div>`;
    ...
    if (qrTextInput && qrSizeInput) {
        qrTextInput.addEventListener('input', updateQRPreview);
        qrSizeInput.addEventListener('input', updateQRPreview);
    }
    // QRコード生成フォームsubmit
    const qrForm = document.getElementById('qr-create-form');
    if (qrForm) {
        qrForm.onsubmit = function(ev) {
            ev.preventDefault();
            const text = qrTextInput.value;
            const size = parseInt(qrSizeInput.value) || 256;
            // QRコードをcanvasで生成し、dataURL化
            ...
            // QRCode.jsはcanvasまたはimgを生成するのでimgを取得
            setTimeout(() => {
                const img = tmpDiv.querySelector('img');
                if (img && img.src) {
                    if (window.App && App.addElement) {
                        App.addElement('image', img.src);
                    }
    ...
    if (form) {
        form.onsubmit = function(ev) {
            ...
            if (document.getElementById('chart-colors').value.trim() !== '') {
                colors = document.getElementById('chart-colors').value.split(',').map(s => s.trim()).filter(Boolean);
            }
            if (colors.length < dataValues.length) {
                colors = colors.concat([
                    '#007bff', '#28a745', '#dc3545', '#ffc107', '#6f42c1',
                    '#e83e8c', '#20c997', '#fd7e14', '#6c757d', '#0dcaf0'
                ].slice(0, dataValues.length - colors.length));
            }
            // Appがwindowにある前提
...
const App = {
    ...
    cacheElements() {
        this.elements = {
            ...
            addTextBtn: document.getElementById('add-text-btn'), addImageBtn: document.getElementById('add-image-btn'), addChartBtn: document.getElementById('add-chart-btn'), addIframeBtn: document.getElementById('add-iframe-btn'), saveBtn: document.getElementById('save-btn'),
            ...
            faIconListContainer: document.getElementById('fa-icon-list-container'), // New
            faIconSearchInput: document.getElementById('fa-icon-search-input'), // New
            faIconCategoryFilter: document.getElementById('fa-icon-category-filter'), // New for category filter
            miIconListContainer: document.getElementById('mi-icon-list-container'), // New
            miIconSearchInput: document.getElementById('mi-icon-search-input'), // New
            miIconCategoryFilter: document.getElementById('mi-icon-category-filter'), // New for category filter
    ...
    config: {
        fontAwesomeIcons: [
            // 一般
            { name: '家', category: '一般', class: 'fas fa-house' }, { name: '虫眼鏡', category: '一般', class: 'fas fa-magnifying-glass' },
            { name: 'ユーザー', category: '一般', class: 'fas fa-user' }, { name: '歯車', category: '一般', class: 'fas fa-gear' },
            { name: '星', category: '一般', class: 'fas fa-star' }, { name: 'ハート', category: '一般', class: 'fas fa-heart' },
            { name: '封筒', category: '一般', class: 'fas fa-envelope' }, { name: '旗', category: '一般', class: 'fas fa-flag' },
            { name: 'ベル', category: '一般', class: 'fas fa-bell' }, { name: 'タグ', category: '一般', class: 'fas fa-tag' },
            { name: '地球', category: '一般', class: 'fas fa-globe' }, { name: '地図', category: '一般', class: 'fas fa-map' },
            { name: 'カレンダー', category: '一般', class: 'fas fa-calendar' }, { name: 'カメラ', category: '一般', class: 'fas fa-camera' },
            { name: '電球', category: '一般', class: 'fas fa-lightbulb' }, { name: '本', category: '一般', class: 'fas fa-book' },
            { name: 'ギフト', category: '一般', class: 'fas fa-gift' }, { name: '鍵', category: '一般', class: 'fas fa-key' },
            { name: '印刷', category: '一般', class: 'fas fa-print' }, { name: '雲', category: '一般', class: 'fas fa-cloud' },
            { name: '時計', category: '一般', class: 'fas fa-clock' }, { name: 'リンク', category: '一般', class: 'fas fa-link' },
            { name: 'ゴミ箱', category: '一般', class: 'fas fa-trash' }, { name: '編集', category: '一般', class: 'fas fa-pen' },
            { name: '共有', category: '一般', class: 'fas fa-share' }, { name: 'ダウンロード', category: '一般', class: 'fas fa-download' },
            { name: 'アップロード', category: '一般', class: 'fas fa-upload' }, { name: 'ホーム', category: '一般', class: 'fas fa-home' },
            { name: '設定', category: '一般', class: 'fas fa-cog' }, { name: 'リロード', category: '一般', class: 'fas fa-sync' },
            { name: '外部サイト', category: '一般', class: 'fas fa-external-link-alt' }, { name: 'ノート', category: '一般', class: 'fas fa-sticky-note' },
            { name: '地球儀', category: '一般', class: 'fas fa-globe-americas' }, { name: '地球ヨーロッパ', category: '一般', class: 'fas fa-globe-europe' },
            // 矢印
            { name: '左矢印', category: '矢印', class: 'fas fa-arrow-left' }, { name: '右矢印', category: '矢印', class: 'fas fa-arrow-right' },
            { name: '上矢印', category: '矢印', class: 'fas fa-arrow-up' }, { name: '下矢印', category: '矢印', class: 'fas fa-arrow-down' },
            { name: '回転矢印', category: '矢印', class: 'fas fa-arrows-rotate' }, { name: 'ダブル左矢印', category: '矢印', class: 'fas fa-angle-double-left' },
            { name: 'ダブル右矢印', category: '矢印', class: 'fas fa-angle-double-right' }, { name: 'ダブル上矢印', category: '矢印', class: 'fas fa-angle-double-up' },
            { name: 'ダブル下矢印', category: '矢印', class: 'fas fa-angle-double-down' }, { name: '矢印円', category: '矢印', class: 'fas fa-arrow-circle-right' },
            // ビジネス・金融
            { name: 'ブリーフケース', category: 'ビジネス', class: 'fas fa-briefcase' }, { name: '折れ線グラフ', category: 'ビジネス', class: 'fas fa-chart-line' },
            { name: '円グラフ', category: 'ビジネス', class: 'fas fa-chart-pie' }, { name: 'ビル', category: 'ビジネス', class: 'fas fa-building' },
            { name: '拡声器', category: 'ビジネス', class: 'fas fa-bullhorn' }, { name: 'ショッピングカート', category: 'ビジネス', class: 'fas fa-shopping-cart' },
            { name: 'クレジットカード', category: 'ビジネス', class: 'fas fa-credit-card' }, { name: '円マーク', category: 'ビジネス', class: 'fas fa-yen-sign' },
            { name: 'ドルマーク', category: 'ビジネス', class: 'fas fa-dollar-sign' }, { name: '棒グラフ', category: 'ビジネス', class: 'fas fa-chart-bar' },
            { name: '財布', category: 'ビジネス', class: 'fas fa-wallet' }, { name: '請求書', category: 'ビジネス', class: 'fas fa-file-invoice' },
            // コミュニケーション
            { name: 'コメント', category: 'コミュニケーション', class: 'fas fa-comment' }, { name: '複数コメント', category: 'コミュニケーション', class: 'fas fa-comments' },
            { name: '電話', category: 'コミュニケーション', class: 'fas fa-phone' }, { name: 'アットマーク', category: 'コミュニケーション', class: 'fas fa-at' },
            { name: 'メール', category: 'コミュニケーション', class: 'fas fa-envelope-open' }, { name: 'チャット', category: 'コミュニケーション', class: 'fas fa-comment-dots' },
            { name: 'マイク', category: 'コミュニケーション', class: 'fas fa-microphone' }, { name: 'ビデオ', category: 'コミュニケーション', class: 'fas fa-video' },
            { name: 'モバイル', category: 'コミュニケーション', class: 'fas fa-mobile-alt' }, { name: 'PC', category: 'コミュニケーション', class: 'fas fa-desktop' },
            { name: 'タブレット', category: 'コミュニケーション', class: 'fas fa-tablet-alt' }, { name: 'SNS', category: 'コミュニケーション', class: 'fas fa-hashtag' },
            // デザイン
            { name: 'パレット', category: 'デザイン', class: 'fas fa-palette' }, { name: 'ペンと定規', category: 'デザイン', class: 'fas fa-pen-ruler' },
            { name: 'オブジェクトグループ', category: 'デザイン', class: 'fas fa-object-group' }, { name: '切り抜き', category: 'デザイン', class: 'fas fa-crop' },
            { name: 'レイヤーグループ', category: 'デザイン', class: 'fas fa-layer-group' }, { name: 'ブラシ', category: 'デザイン', class: 'fas fa-paint-brush' },
            { name: '消しゴム', category: 'デザイン', class: 'fas fa-eraser' }, { name: '色見本', category: 'デザイン', class: 'fas fa-eye-dropper' },
            // 食べ物
            { name: '食器', category: '食べ物', class: 'fas fa-utensils' }, { name: 'カクテルグラス', category: '食べ物', class: 'fas fa-martini-glass-citrus' },
            { name: 'ピザ', category: '食べ物', class: 'fas fa-pizza-slice' }, { name: 'ハンバーガー', category: '食べ物', class: 'fas fa-burger' },
            { name: 'アイスクリーム', category: '食べ物', class: 'fas fa-ice-cream' }, { name: 'コーヒー', category: '食べ物', class: 'fas fa-coffee' },
            { name: 'ケーキ', category: '食べ物', class: 'fas fa-birthday-cake' }, { name: 'りんご', category: '食べ物', class: 'fas fa-apple-alt' },
            // 旅行
            { name: '飛行機', category: '旅行', class: 'fas fa-plane' }, { name: '車', category: '旅行', class: 'fas fa-car' },
            { name: '船', category: '旅行', class: 'fas fa-ship' }, { name: '地球', category: '旅行', class: 'fas fa-earth-americas' },
            { name: '地図', category: '旅行', class: 'fas fa-map-location-dot' }, { name: 'ホテル', category: '旅行', class: 'fas fa-hotel' },
            { name: 'バス', category: '旅行', class: 'fas fa-bus' }, { name: '電車', category: '旅行', class: 'fas fa-train' },
            { name: 'タクシー', category: '旅行', class: 'fas fa-taxi' }, { name: '山', category: '旅行', class: 'fas fa-mountain' },
            // 天気
            { name: '太陽', category: '天気', class: 'fas fa-sun' }, { name: '雲', category: '天気', class: 'fas fa-cloud' },
            { name: '雪の結晶', category: '天気', class: 'fas fa-snowflake' }, { name: '雷', category: '天気', class: 'fas fa-cloud-bolt' },
            { name: '風', category: '天気', class: 'fas fa-wind' }, { name: '雨', category: '天気', class: 'fas fa-cloud-showers-heavy' },
            { name: '虹', category: '天気', class: 'fas fa-rainbow' }, { name: '温度計', category: '天気', class: 'fas fa-thermometer-half' },
            // モノ
            { name: 'カメラ', category: 'モノ', class: 'fas fa-camera' }, { name: '電球', category: 'モノ', class: 'fas fa-lightbulb' },
            { name: '本', category: 'モノ', class: 'fas fa-book' }, { name: 'ギフト', category: 'モノ', class: 'fas fa-gift' },
            { name: '鍵', category: 'モノ', class: 'fas fa-key' }, { name: '音楽', category: 'モノ', class: 'fas fa-music' },
            { name: 'ゲーム', category: 'モノ', class: 'fas fa-gamepad' }, { name: 'トロフィー', category: 'モノ', class: 'fas fa-trophy' },
            { name: '卒業帽', category: 'モノ', class: 'fas fa-graduation-cap' }, { name: '傘', category: 'モノ', class: 'fas fa-umbrella' },
            { name: '自転車', category: 'モノ', class: 'fas fa-bicycle' }, { name: '車椅子', category: 'モノ', class: 'fas fa-wheelchair' },
            // 絵文字
            { name: '笑顔', category: '絵文字', class: 'fas fa-face-smile' }, { name: '爆笑', category: '絵文字', class: 'fas fa-face-laugh' },
            { name: 'ウィンク', category: '絵文字', class: 'fas fa-face-grin-wink' }, { name: '泣き顔', category: '絵文字', class: 'fas fa-face-sad-tear' },
            { name: '怒り顔', category: '絵文字', class: 'fas fa-face-angry' }, { name: '驚き顔', category: '絵文字', class: 'fas fa-face-surprise' },
            // 追加アイコン
            { name: 'チェック', category: '一般', class: 'fas fa-check' }, { name: 'バツ', category: '一般', class: 'fas fa-times' },
            { name: 'プラス', category: '一般', class: 'fas fa-plus' }, { name: 'マイナス', category: '一般', class: 'fas fa-minus' },
            { name: '情報', category: '一般', class: 'fas fa-info-circle' }, { name: '質問', category: '一般', class: 'fas fa-question-circle' },
            { name: '警告', category: '一般', class: 'fas fa-exclamation-triangle' }, { name: 'カレンダー', category: '一般', class: 'fas fa-calendar-alt' },
            { name: '時計', category: '一般', class: 'fas fa-clock' }, { name: '印刷', category: '一般', class: 'fas fa-print' },
            { name: 'リンク', category: '一般', class: 'fas fa-link' }, { name: 'ゴミ箱', category: '一般', class: 'fas fa-trash-alt' },
            { name: '編集', category: '一般', class: 'fas fa-edit' }, { name: '共有', category: '一般', class: 'fas fa-share-square' },
            { name: 'ダウンロード', category: '一般', class: 'fas fa-download' }, { name: 'アップロード', category: '一般', class: 'fas fa-upload' },
            { name: 'ホーム', category: '一般', class: 'fas fa-home' }, { name: '設定', category: '一般', class: 'fas fa-cog' },
            { name: 'リロード', category: '一般', class: 'fas fa-sync-alt' }, { name: '外部サイト', category: '一般', class: 'fas fa-external-link-alt' },
            { name: 'ショッピングカート', category: 'ビジネス', class: 'fas fa-shopping-cart' }, { name: 'クレジットカード', category: 'ビジネス', class: 'fas fa-credit-card' },
            { name: '円マーク', category: 'ビジネス', class: 'fas fa-yen-sign' }, { name: 'ドルマーク', category: 'ビジネス', class: 'fas fa-dollar-sign' },
            { name: '棒グラフ', category: 'ビジネス', class: 'fas fa-chart-bar' },
            { name: 'マイク', category: 'コミュニケーション', class: 'fas fa-microphone' }, { name: 'ビデオ', category: 'コミュニケーション', class: 'fas fa-video' },
            { name: 'モバイル', category: 'コミュニケーション', class: 'fas fa-mobile-alt' }, { name: 'PC', category: 'コミュニケーション', class: 'fas fa-desktop' },
            { name: 'タブレット', category: 'コミュニケーション', class: 'fas fa-tablet-alt' },
            { name: '音楽', category: 'モノ', class: 'fas fa-music' }, { name: 'ゲーム', category: 'モノ', class: 'fas fa-gamepad' },
            { name: 'トロフィー', category: 'モノ', class: 'fas fa-trophy' }, { name: '卒業帽', category: 'モノ', class: 'fas fa-graduation-cap' },
        ],
        materialIcons: [
            { name: 'ホーム', category: '一般', class: 'home' },
            { name: '検索', category: '一般', class: 'search' },
            { name: '設定', category: '一般', class: 'settings' },
            { name: '情報', category: '一般', class: 'info' },
            { name: '警告', category: '一般', class: 'warning' },
            { name: 'エラー', category: '一般', class: 'error' },
            { name: 'チェック', category: '一般', class: 'check_circle' },
            { name: '閉じる', category: '一般', class: 'close' },
            { name: '削除', category: '一般', class: 'delete' },
            { name: '編集', category: '一般', class: 'edit' },
            { name: 'メール', category: 'コミュニケーション', class: 'mail' },
            { name: '電話', category: 'コミュニケーション', class: 'phone' },
            { name: 'チャット', category: 'コミュニケーション', class: 'chat' },
            { name: 'ビデオカメラ', category: 'コミュニケーション', class: 'videocam' },
            { name: '写真', category: 'メディア', class: 'photo' },
            { name: '画像', category: 'メディア', class: 'image' },
            { name: '動画', category: 'メディア', class: 'movie' },
            { name: '音楽', category: 'メディア', class: 'music_note' },
            { name: 'プレイ', category: 'メディア', class: 'play_arrow' },
            { name: '一時停止', category: 'メディア', class: 'pause' },
            { name: '停止', category: 'メディア', class: 'stop' },
            { name: 'ダウンロード', category: 'ファイル', class: 'download' },
            { name: 'アップロード', category: 'ファイル', class: 'upload' },
            { name: 'フォルダ', category: 'ファイル', class: 'folder' },
            { name: 'ドキュメント', category: 'ファイル', class: 'description' },
            { name: 'カレンダー', category: '日付と時刻', class: 'calendar_today' },
            { name: '時計', category: '日付と時刻', class: 'access_time' },
            { name: '場所', category: 'マップ', class: 'place' },
            { name: 'ナビ', category: 'マップ', class: 'navigation' },
            { name: 'バス', category: '交通', class: 'directions_bus' },
            { name: '車', category: '交通', class: 'directions_car' },
            { name: '電車', category: '交通', class: 'directions_railway' },
            { name: 'フライト', category: '交通', class: 'flight' },
            { name: 'ショッピングカート', category: 'Eコマース', class: 'shopping_cart' },
            { name: '支払い', category: 'Eコマース', class: 'payment' },
            { name: 'アチーブメント', category: 'アチーブメント', class: 'stars' },
            { name: 'トロフィー', category: 'アチーブメント', class: 'emoji_events' },
            { name: 'ギフト', category: 'アチーブメント', class: 'card_giftcard' },
            { name: 'イベント', category: 'イベント', class: 'event' },
            { name: '通知', category: '通知', class: 'notifications' },
            { name: 'アラーム', category: '通知', class: 'alarm' },
            { name: 'バッテリー', category: 'デバイス', class: 'battery_full' },
            { name: 'Wi-Fi', category: 'デバイス', class: 'wifi' },
            { name: 'Bluetooth', category: 'デバイス', class: 'bluetooth' },
            { name: '明るさ', category: '画像調整', class: 'brightness_6' },
            { name: 'コントラスト', category: '画像調整', class: 'contrast' },
            { name: '彩度', category: '画像調整', class: 'saturation_on' },
            { name: 'フィルター', category: '画像調整', class: 'filter_vintage' },
            { name: '農場', category: '自然', class: 'grass' },
            { name: '山', category: '自然', class: 'landscape' },
            { name: '木', category: '自然', class: 'forest' },
            { name: '太陽', category: '天気', class: 'wb_sunny' },
            { name: '雲', category: '天気', class: 'cloud' },
            { name: '雨', category: '天気', class: 'umbrella' },
            { name: '雪', category: '天気', class: 'ac_unit' },
            { name: '嵐', category: '天気', class: 'flash_on' },
            { name: '心臓', category: 'ヘルスケア', class: 'favorite' },
            { name: '医療', category: 'ヘルスケア', class: 'local_hospital' },
            { name: '救急車', category: 'ヘルスケア', class: 'ambulance' },
            { name: '薬', category: 'ヘルスケア', class: 'medical_services' },
            { name: 'レシピ', category: '調理', class: 'restaurant_menu' },
            { name: '調理器具', category: '調理', class: 'kitchen' },
            { name: 'カフェ', category: '調理', class: 'local_cafe' },
            { name: 'バー', category: '調理', class: 'local_bar' },
            { name: 'スポーツ', category: 'スポーツ', class: 'sports_soccer' },
            { name: 'フィットネス', category: 'スポーツ', class: 'fitness_center' },
            { name: 'ランニング', category: 'スポーツ', class: 'directions_run' },
            { name: '自転車', category: 'スポーツ', class: 'directions_bike' },
            { name: '犬', category: '動物', class: 'pets' },
            { name: '猫', category: '動物', class: 'cat' },
            { name: '鳥', category: '動物', class: 'bird' },
            { name: '魚', category: '動物', class: 'fish' },
            { name: 'ロケット', category: 'サイエンス', class: 'rocket' },
            { name: 'フラスコ', category: 'サイエンス', class: 'science' },
            { name: '電球', category: 'サイエンス', class: 'lightbulb' },
            { name: '虫眼鏡', category: 'サイエンス', class: 'search' },
            { name: '電卓', category: 'ビジネス', class: 'calculate' },
            { name: 'ポートフォリオ', category: 'ビジネス', class: 'work' },
            { name: '会議', category: 'ビジネス', class: 'meeting_room' },
            { name: 'プレゼンテーション', category: 'ビジネス', class: 'slideshow' },
            { name: '学校', category: '教育', class: 'school' },
            { name: '本', category: '教育', class: 'book' },
            { name: '鉛筆', category: '教育', class: 'create' },
            { name: '卒業', category: '教育', class: 'school' },
            { name: '盾', category: 'セキュリティ', class: 'security' },
            { name: 'ロック', category: 'セキュリティ', class: 'lock' },
            { name: '鍵', category: 'セキュリティ', class: 'vpn_key' },
            { name: '指紋', category: 'セキュリティ', class: 'fingerprint' },
            { name: 'ゲームパッド', category: 'ゲーム', class: 'gamepad' },
            { name: 'コントローラー', category: 'ゲーム', class: 'sports_esports' },
            { name: 'サイコロ', category: 'ゲーム', class: 'casino' },
            { name: 'パズル', category: 'ゲーム', class: 'extension' },
            { name: '車椅子', category: 'アクセシビリティ', class: 'accessible_forward' },
            { name: '点字', category: 'アクセシビリティ', class: 'braille' },
            { name: '手話', category: 'アクセシビリティ', class: 'sign_language' },
            { name: '音声', category: 'アクセシビリティ', class: 'record_voice_over' },
            { name: '顔', category: '絵文字', class: 'face' },
            { name: '笑顔', category: '絵文字', class: 'sentiment_satisfied_alt' },
            { name: '泣き顔', category: '絵文字', class: 'sentiment_dissatisfied' },
            { name: '怒り顔', category: '絵文字', class: 'sentiment_very_dissatisfied' },
            { name: '眠り顔', category: '絵文字', class: 'sentiment_neutral' },
            { name: 'ハグ', category: '絵文字', class: 'sentiment_very_satisfied' },
        ]
    },
...
createElementDOM(elData) {
    ...
    if (elData.type === 'text') {
        ...
        if (elData.iconType === 'fa') {
            ...
            iTag.style.width = '100%';
            iTag.style.height = '100%';
            el.style.overflow = 'visible';
            el.appendChild(iTag);
        } else if (elData.iconType === 'mi') {
            const spanTag = document.createElement('span');
            spanTag.className = elData.content; // e.g., "material-icons" or "material-icons-outlined"
            spanTag.textContent = elData.miContent; // e.g., "home"
        ...
        Object.assign(overlay.style, {
            ...
            cursor: 'move', // 移動カーソルを表示
        });
        // オーバーレイがクリックされたら要素を選択状態にする
        overlay.addEventListener('mousedown', (e) => {
            // e.stopPropagation(); // 親要素へのイベント伝播を止める
            // e.preventDefault(); // デフォルトのドラッグ動作を止める
            App.handleCanvasMouseDown(e); // Appのmousedownハンドラを呼び出す
        });
                ...
                if (e.target.classList.contains('sidebar-tab-button')) {
                    ...
                    this.elements.sidebarContent.querySelectorAll('.sidebar-tab-content').forEach(content => {
                        if (content.dataset.tabContent === tabName) {
                            ...
                            else if (tabName === 'font-awesome-icons') {
                                this.initCategoryFilters('fa'); // Font Awesomeのフィルタを初期化
                                this.renderIconList('fa'); // Font Awesomeのアイコンリストを描画
                            } else if (tabName === 'material-icons') {
                                this.initCategoryFilters('mi'); // Material Iconsのフィルタを初期化
                                this.renderIconList('mi'); // Material Iconsのアイコンリストを描画
                            }
                        } else {
                            content.classList.remove('active');
                        }
                    });
                    // 初期表示時にアクティブなタブのアイコンリストをロード
                    if (e.target.classList.contains('active')) {
                        const currentTabName = e.target.dataset.tab;
                        if (currentTabName === 'font-awesome-icons') {
                            this.initCategoryFilters('fa');
                            this.renderIconList('fa');
                        } else if (currentTabName === 'material-icons') {
    ...
    addIconElement(iconType, iconClass) {
        ...
        const newEl = {
            ...
            style: {
                top: 20, left: 20, width: 5, height: 5,
